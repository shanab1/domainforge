...
// Get list of sites (CloudPanel 6.x has no site:list command, scan filesystem)
const getSites = () => {
    const sites = [];
    ...
    try {
        ...
        for (const dir of dirs) {
            ...
            // Check for domain subdirectories in htdocs (CloudPanel 6.x structure)
            try {
                const subDirs = fs.readdirSync(htdocsPath);
                for (const sub of subDirs) {
                    const subPath = path.join(htdocsPath, sub);
                    // Domain directories contain dots and are directories
                    if (sub.includes('.') && fs.statSync(subPath).isDirectory()) {
                        sites.push({ domain: sub, type: 'static', user: dir });
                    }
                }
            } catch {}
        }
...
// Request SSL certificate (CloudPanel 6.x uses lets-encrypt:install:certificate)
const requestSSL = domain => {
    return run(`${CLPCTL} lets-encrypt:install:certificate --domainName="${domain}" 2>&1`);
};
...
// Find the actual site directory (CloudPanel uses /home/siteUser/htdocs/domain/)
const findSiteDir = domain => {
    const siteUser = domain.replace(/\./g, '').toLowerCase();
    
    // CloudPanel structure: /home/{siteUser}/htdocs/{domain}/
    let sitePath = path.join(SITES_PATH, siteUser, 'htdocs', domain);
    if (fs.existsSync(sitePath)) return sitePath;
    
    // Try without the full domain path (older CloudPanel versions)
    sitePath = path.join(SITES_PATH, siteUser, 'htdocs');
    if (fs.existsSync(sitePath) && !fs.existsSync(path.join(sitePath, domain))) {
        return sitePath;
    }
    
    // Try exact domain match as directory name
    sitePath = path.join(SITES_PATH, domain, 'htdocs');
    if (fs.existsSync(sitePath)) return sitePath;
    
    // Search for directory containing the domain
    try {
        const dirs = fs.readdirSync(SITES_PATH);
        for (const dir of dirs) {
            // Check /home/{dir}/htdocs/{domain}/
            const htdocsDomain = path.join(SITES_PATH, dir, 'htdocs', domain);
            if (fs.existsSync(htdocsDomain)) {
                return htdocsDomain;
            }
            // Check /home/{dir}/htdocs/
...
// Extract ZIP file to a site (with optional subfolder)
const extractZip = (domain, filename, base64Content, folder = '') => {
    ...
    // Determine target path (htdocs or htdocs/folder)
    let targetPath = basePath;
    if (folder) {
        // Sanitize folder name (remove path traversal attempts)
