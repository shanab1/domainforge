...
const createSite = domain => {
    ...
    // Try with siteUser parameter (required in newer CloudPanel versions)
    let r = run(`${CLPCTL} site:add:static --domainName="${domain}" --siteUser="${siteUser}" 2>&1`);
    if (!r.ok) {
        // Try alternative syntax
        r = run(`${CLPCTL} site:add --domainName="${domain}" --siteUser="${siteUser}" --siteType="static" 2>&1`);
    }
    return r;
};
...
const enablePageSpeed = domain => {
    return run(`${CLPCTL} site:pagespeed:enable --domainName="${domain}" 2>&1`);
};

// Write a file to a site
const writeFile = (domain, filePath, content) => {
    const basePath = path.join(SITES_PATH, domain, 'htdocs');
