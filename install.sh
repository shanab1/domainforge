...
// HTTP server on port 8080 (for CORS proxy compatibility)
...
log(`HTTPS: https://0.0.0.0:${PORT}`);
log(`HTTP:  http://0.0.0.0:${HTTP_PORT} (use this with CORS proxy)`);
...
if command -v ufw &> /dev/null; then
    ...
    echo "       Ports 8443 and 8080 opened"
fi
...
echo "  Port:      8080  (use this one!)"
echo "  Token:     $TOKEN"
echo ""
echo "  Copy these into DomainForge Integrations!"
echo ""
echo "  IMPORTANT: Use port 8080 (HTTP) not 8443"
echo "  The CORS proxy cannot handle self-signed HTTPS"
echo ""
echo "  Test locally:"
